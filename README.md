...
- We fuel-correct lap times in order to estimate tyre degradation. The Fuel-corrected lap time $\overline{L}$ can be computed from the actual lap time $L$ as $\overline{L}_i = L_i - (66-i+0.5)*f$. Notice that $\int_0^1 66-(i-1)-x \ dx=\frac{66-(i-1)+66-i}{2} = 66-i+0.5$ gives the average amount of laps of fuel on board during lap $i$, assuming linear fuel burning.
